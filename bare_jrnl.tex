...
In [39], a curve representation method was proposed based on a sequence of tangent angles, 
but this method is not invariant under either translation or rotation. 
In [40], a type of turning function was proposed to represent polygons and to measure their shapes, 
but this method is very sensitive to small zigzag or burr noise, 
and, worse yet, representation errors in the front part [ACCDON55]will accumulate and propagate to the tail part. 
In addition, the algorithm requires normalization in the scope of the entire polygon[ACCDON56], 
which consolidates the relative length proportions of all edges. 
This is not suitable for those curves suffering from a degree of mixing with the background or those that lack some portion of a normal contour. 
In [41], a discrete curve evolution algorithm, which uses polygons to approximate curves, was proposed. 
However, this method introduces a fitting error while initializing the number of polygon edges, 
and, moreover, requires computation in addition to calculations for evaluation of polygon similarity[ACCDON57]. 
Matlab also provides a function call for calculation of curvature, 
but its return value is seriously affected by local noise. 
B-spline can be effectively used to fit a curve in advance, 
but this requires an accurate sequence of sampling sites. 
This sequence cannot be guaranteed on a new image, and manual appointment of a sequence is not possible. 
