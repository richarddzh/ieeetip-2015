...
and establish the links between those nodes (i.e., chips) that might be successively joined together to represent a slightly longer line or curve. 
...
Therefore, no links can be established between these independent nodes. 
...
The present description defines a basic network of chip nodes, i.e., an orientation chip graph. 
Once an edge image has been uploaded, 
some nodes of this graph will be activated. 
Most importantly, the chip graph provides a structural data form for subsequent geometrical or topological analysis, as illustrated in \figurename~\ref{fig:8}.
...
representing a continous contour requires the coordination of multiple chips, 
...
Route-searching seeks to combine the neighboring nodes of a graph. 
However, some difficulties are encountered in this process because the contour of an object can vary, 
such as a giraffe might stretch or bend its neck, 
or a contour might suffer from interference with its background and be mixed with noise or be broken into several shorter segments. 
