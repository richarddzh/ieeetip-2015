...
In \figurename~\ref{fig:23a}, a suspected route is selected as the beginning of the search because it matches a part of the template well. 
...
In \figurename~\ref{fig:23b}, through loosening the threshold defining neighboring nodes, 
...
making the second candidate the better choice, which leads us to \figurename~\ref{fig:23c}. 
From \figurename~\ref{fig:23d}~to~\ref{fig:23e} the remaining nodes can be evaluated in the same manner, 
where only the node with the smallest heuristic value can be extended. 
In particular, \figurename~\ref{fig:23e} indicates that a region-limited preprocessing is executed again under a new canny-detector parameter because the active processing attempts to find a permanently lost edge that bridges the gap between two routes. 
Finally, as shown in \figurename~\ref{fig:23f}, an almost perfect contour of a bottle is fully detected.
