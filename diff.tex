...
%DIF DEL bare_jrnl_old.tex   Sat Nov  7 21:09:09 2015
%DIF ADD bare_jrnl.tex       Thu Nov 19 10:43:51 2015
...
\DIFaddbegin \DIFadd{One of the fundamental functions of V1 area is extracting features about edges and orientations from the visual stimuli.
...
\mbox{%DIFAUXCMD
\cite{canny1986,arbelaez2011,moreno2014}}%DIFAUXCMD
.
Canny detector \mbox{%DIFAUXCMD
\cite{canny1986} }%DIFAUXCMD
and the globalPb \mbox{%DIFAUXCMD
\cite{arbelaez2011} }%DIFAUXCMD
generate edge maps,
which represent edges in the level of pixels. 
The tensor voting \mbox{%DIFAUXCMD
\cite{moreno2014} }%DIFAUXCMD
and the LSD method \mbox{%DIFAUXCMD
\cite{von2008} }%DIFAUXCMD
...
\DIFaddend \subsection{Shape- or contour-based object recognition}
...
was a frequently-used method that \DIFdelbegin \DIFdel{converted }\DIFdelend \DIFaddbegin \DIFadd{transformed }\DIFaddend an original image into a \DIFdelbegin \DIFdel{depth image}\DIFdelend \DIFaddbegin \DIFadd{distance map}\DIFaddend , 
...
We \DIFdelbegin \DIFdel{hope }\DIFdelend \DIFaddbegin \DIFadd{wish }\DIFaddend to represent a continuous contour by coordinating the information derived from multiple chips through the implementation of a graph. 
...
\DIFdelbegin \DIFdel{Once an image has been uploaded, 
some }\DIFdelend \DIFaddbegin \DIFadd{Some }\DIFaddend nodes of this graph will be activated \DIFaddbegin \DIFadd{when an image is presented to the hypercolumn array}\DIFaddend . 
...
\cite{wang1997}}%DIFAUXCMD
, the dynamic time warping (DTW) method has been adopted to curve alignment,
but his method is not applicable to curves that can not be presented with one dimensional functions.
}\DIFaddend 

\DIFdelbegin \DIFdel{Intuitively, the most direct method of comparing two curves is to evaluate the degree to which they are mutually overlapping,
which is similar to solving an optimization problem, i.
e., finding the best-matched interval for two curves.
...
Furthermore, this method can flexibly adapt to curve variances of length, 
position, and posture. 
We therefore designed a curve similarity measurement algorithm based on evaluating the degree of overlap between two curves}\DIFdelend \DIFaddbegin \DIFadd{In this paper, the curve similarity is measured with a machine learning approach based on 
the non-linearly embedded extreme learning machine (NLE-ELM) \mbox{%DIFAUXCMD
\cite{liu2015}}%DIFAUXCMD
.
The extreme learning machine (ELM) \mbox{%DIFAUXCMD
\cite{huang2006,tang2015,mohammed2011,huang2014}
}%DIFAUXCMD
...
It inherits the merits of ELM, such as avoiding local optima, fast learning speed}\DIFaddend . 

\DIFdelbegin \DIFdel{Here, we are concerned with two classes of curves. 
...
}%DIFDELCMD < \figurename%%%
\DIFdel{~\ref{fig:12} illustrates our idea, and the algorithm is given as follows.
...
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.7\linewidth]{images/fig12.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{An example of a similarity measurement of two curves.}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.9\linewidth]{images/fig12.jpg}
\caption{\DIFaddFL{Representing routes with vectors.
The routes in the images are transformed into vectors plotted below the images.}}
\DIFaddendFL \label{fig:12}
\end{figure}

\DIFdelbegin %DIFDELCMD < \begin{myalgorithm}{Algorithm 5.1 Route-Distance($r,t$)}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The routes are represented with vectors which are provided as input to the NLE-ELM algorithm.
...
Examples of the vectors are shown in }\figurename\DIFadd{~\ref{fig:12}.
In each image, two routes around the contour of the apple logo are chosen to be represented into vectors of orientation.
}

\DIFadd{We train an NLE-ELM model to recognize routes from a number $M$ of object categories.
...
\DIFaddend \begin{algorithmic}[1]
\REQUIRE \DIFdelbegin \DIFdel{$r$ is RouteClass curve and $t$ is TempClass curve.
}%DIFDELCMD < \STATE %%%
\DIFdel{rotate $r$ }\DIFdelend \DIFaddbegin \DIFadd{$\boldsymbol{V}$ }\DIFaddend and \DIFdelbegin \DIFdel{$t$ so that their most extended direction is along the $x$-axis
}%DIFDELCMD < \STATE %%%
\DIFdel{sample $t$ uniformly to }\DIFdelend \DIFaddbegin \DIFadd{$\boldsymbol{V}$ are the training data.
$\boldsymbol{V}=(\boldsymbol{v}_1,\boldsymbol{v}_2,\cdots,\boldsymbol{v}_N)$
consists of }\DIFaddend $N$ \DIFdelbegin \DIFdel{points $P_t=\{(x_i,y_i)|i=1,\cdots,N\}$
}\DIFdelend \DIFaddbegin \DIFadd{route vectors, and $\boldsymbol{T}=(\boldsymbol{t}_1,\boldsymbol{t}_2,\cdots,\boldsymbol{t}_N)$
consists of the corresponding indicator vectors.
$C,\gamma_g$ and $\mu$ are parameters of NLE-ELM model.
}\ENSURE \DIFadd{the algorithm calculates the optimized output weight matrix $\boldsymbol\beta^{*}$
of the NLE-ELM model.
}\DIFaddend \STATE \DIFdelbegin \DIFdel{randomly initialize $M$ subintervals of $[0,length(r)]$
}%DIFDELCMD < \REPEAT
%DIFDELCMD <   \FOR{each subinterval $[p,q]$}
%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \DIFadd{Initialize the hidden layer with random input weights
}\DIFaddend \STATE \DIFdelbegin \DIFdel{sample $r$ in $[p,q]$ to $N$ points $P_r=\{(x'_i,y'_i)\}$
    }\DIFdelend \DIFaddbegin \DIFadd{Calculates the output of hidden layer $(\boldsymbol{H,H'})$
}\DIFaddend \STATE \DIFdelbegin \DIFdel{calculate dissimilarity $d(P_t,P_r)$ by solving least-squares
  }%DIFDELCMD < \ENDFOR
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \DIFadd{Let $L$ denotes the number of units in hidden layer
}\IF {\DIFadd{$L\le N$}}
  \DIFaddend \STATE \DIFdelbegin \DIFdel{remove 2 subintervals with maximal dissimilarity
  }\DIFdelend \DIFaddbegin \DIFadd{$\boldsymbol{L_H}=\boldsymbol{I}_N-\boldsymbol{H}'(\gamma_g\boldsymbol{I}_L+\boldsymbol{H'^T H'})^{-1}\boldsymbol{H'^T}$
  }\DIFaddend \STATE \DIFdelbegin \DIFdel{add 2 subintervals by shrinking and extending the one with minimal dissimilarity
}%DIFDELCMD < \UNTIL{the minimal dissimilarity is steady}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{$\boldsymbol\beta^{*}=\left(\frac{\boldsymbol{I}_L}{C}+\boldsymbol{H^T H}+\frac{\mu}{C}\boldsymbol{H^T L_H H}\right)^{-1}\boldsymbol{H^T T}$
}\ELSE
  \STATE \DIFadd{$\boldsymbol{L_H}=\boldsymbol{I}_N-\boldsymbol{H'H^T}(\gamma_g\boldsymbol{I}_N+\boldsymbol{H'H'^T})^{-1}$
  }\STATE \DIFadd{$\boldsymbol\beta^{*}=\boldsymbol{H^T}\left(\frac{\boldsymbol{I}_N}{C}+\boldsymbol{HH^T}+\frac{\mu}{C}\boldsymbol{HH^T L_H}\right)^{-1}\boldsymbol{T}$
}\ENDIF
\DIFaddend \RETURN \DIFdelbegin \DIFdel{the minimal dissimilarity
}\DIFdelend \DIFaddbegin \DIFadd{$\boldsymbol\beta^{*}$
}\DIFaddend \end{algorithmic}
\end{myalgorithm}

\DIFdelbegin \DIFdel{At the beginning of this algorithm,
we rotate a curve and make its most extended direction coincide with the positive $x$-axis.
...
\DIFdel{d(P_t,P_r)=\min_{\Delta x,\Delta y,k}\sum_{i=1}^{N}(kx_i+\Delta x-x'_i)^2+(ky_i+\Delta y-y'_i)^2,
}\end{displaymath}
%DIFAUXCMD
\DIFdel{where $P_t,P_r$ is }\DIFdelend \DIFaddbegin \DIFadd{Given the aforementioned route representation and NLE-ELM model,
...
The output function of }\DIFaddend the \DIFdelbegin \DIFdel{sequences of sampling points on the template curve and route curve respectively.
}\DIFdelend \DIFaddbegin \DIFadd{NLE-ELM model is $f(\boldsymbol{v},\boldsymbol\beta^{*})$, 
...
\DIFadd{\text{Route-Distance}(r,t)=||f(\boldsymbol{v}_r,\boldsymbol\beta^{*})-f(\boldsymbol{v}_t,\boldsymbol\beta^{*})||.
}\end{equation}

\DIFaddend \figurename~\ref{fig:13} presents an example of a similarity comparison between two curves,
...
\DIFaddbegin \DIFadd{The number of routes is exponentially less than the number of line segments and edge pixels.
}\DIFaddend We can see that the degree of image compactness is steadily improved, 
...
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.6\linewidth]{images/fig17.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{A coordinate system where a 3D object is projected upon a 2D plane.}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:17}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{A 2D template is the image of a 3D object projected onto a 2D plane. 
Based on computer graphic theory, we build a coordinate system in }%DIFDELCMD < \figurename%%%
\DIFdel{~\ref{fig:17}, 
...
\DIFdelbegin \DIFdel{The most common method of evaluating object detection performance is through the use of the DR-FPPI curve, 
where DR represents the detection rate, 
and FPPI is the false positives per image.Generally, a small FPPI value can better reflect the performance of an algorithm because the DR can attain a high value provided the FPPI is loosened.
Therefore,
we prefer a smaller FPPI value such as 0.3 or 0.4. 
}%DIFDELCMD < \tablename%%%
\DIFdel{~\ref{tab:1} lists a comparison of DR values obtained by our method and several }\DIFdelend \DIFaddbegin \DIFadd{We test the proposed method on the ETHZ dataset and the INRIA horse dataset for validation.
PASCAL criteria are used for the purpose of detection evaluation, 
i.e., if the detected bounding-box and the ground truth bounding-box intersect over 50\%}{} \DIFadd{of the union of them, 
a detection is counted as correct.
The proposed model is tested on the }\DIFaddend state-of-the-art \DIFdelbegin \DIFdel{methods 
\mbox{%DIFAUXCMD
\cite{ferrari2010,ferrari2008} }%DIFAUXCMD
for FPPI values of 0.2 and 0.4 with images obtained from the ETHZ dataset}\DIFdelend \DIFaddbegin \DIFadd{methods 
\mbox{%DIFAUXCMD
\cite{lu2009,felzenszwalb2007,maji2009,srinivasan2010,wang2012,lin2012}}%DIFAUXCMD
,
we plot the precision-recall (PR) curves in }\figurename\DIFadd{~\ref{fig:17} 
and report the average precision (AP) in }\tablename\DIFadd{~\ref{tab:1}}\DIFaddend .
\tablename~\ref{tab:1} indicates that the performance of our method is either the best or very nearly the best for all \DIFdelbegin \DIFdel{images and FPPR values considered}\DIFdelend \DIFaddbegin \DIFadd{image categories}\DIFaddend .

\begin{table}[!t]
\renewcommand{\arraystretch}{1.3}
\caption{Comparison of \DIFdelbeginFL \DIFdelFL{the detection rate for FPPI 0.2/0.4}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{average precision (AP) on ETHZ Shape classes}\DIFaddendFL }
\label{tab:1}
\centering
\scriptsize
\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{l|lllll}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{l|llllll}
\DIFaddendFL \hline
& Apple & Bottle & Giraffe & Mug & Swan \DIFaddbeginFL & \DIFaddFL{Average }\DIFaddendFL \\
\hline
Our method              & \DIFdelbeginFL \textbf{\DIFdelFL{0.80}}%DIFAUXCMD
\DIFdelFL{/0.83
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.875                 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.78/0.91
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.963         }\DIFaddendFL &\textbf{\DIFdelbeginFL \DIFdelFL{0.77/0.80}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.859}\DIFaddendFL } &\DIFdelbeginFL \DIFdelFL{0.75/}\DIFdelendFL \textbf{\DIFdelbeginFL \DIFdelFL{0.88}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.906}\DIFaddendFL } &\textbf{\DIFdelbeginFL \DIFdelFL{0.75/0.81}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.972}\DIFaddendFL } \DIFaddbeginFL &\textbf{\DIFaddFL{0.915}} \DIFaddendFL \\
\DIFdelbeginFL \DIFdelFL{$\text{Hough only}_\text{(Pascal)}$
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Lu et al.\mbox{%DIFAUXCMD
\cite{lu2009}  }%DIFAUXCMD
}\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.18/0.29
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.844                 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.48/0.56
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.641         }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.16/0.35
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.617         }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.20/0.36
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.643         }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.29/0.46}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.798         }& \DIFaddFL{0.709 }\DIFaddendFL \\
\DIFdelbeginFL \DIFdelFL{$\text{Full system}_\text{(Pascal)}$
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Felz et al.\mbox{%DIFAUXCMD
\cite{felzenszwalb2007} }%DIFAUXCMD
}\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.74/}\textbf{\DIFdelFL{0.84}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.891      }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.69/0.82
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.950         }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.34/0.45
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.608         }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.66/0.79
...
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Maji et al.\mbox{%DIFAUXCMD
\cite{maji2009}  }%DIFAUXCMD
...
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Srinivasan et al.\mbox{%DIFAUXCMD
\cite{srinivasan2010}  }%DIFAUXCMD
...
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Wang et al.\mbox{%DIFAUXCMD
\cite{wang2012} }%DIFAUXCMD
...
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.832         }\DIFaddendFL & \DIFdelbeginFL \textbf{\DIFdelFL{0.78}}%DIFAUXCMD
\DIFdelFL{/0.81
...
\DIFaddbeginFL \DIFaddFL{Lin et al.\mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{The Weizzman dataset of horse images }\DIFdelend \DIFaddbegin \begin{figure}[!t]
\centering
...
\caption{\DIFaddFL{Precision/Recall curves of our method and compared methods on the ETHZ database.}}
\label{fig:17}
\end{figure}

\DIFadd{The INRIA horses dataset }\DIFaddend is another popular dataset for object detection testing. 
\DIFaddbegin \DIFadd{In this dataset, 50 positive examples and 50 negative examples are randomly selected for training. 
The final trained model is tested on the remaining images.
A few representative shape detection results generated by our method is shown in }\DIFaddend \figurename\DIFdelbegin \DIFdel{~\ref{fig:21} presents our testing results on this dataset. 
The three green arrowed curves represent the template segments, 
the blue curves are the successfully matched contours, and the red boxes indicate the positions of horses. }\DIFdelend \DIFaddbegin \DIFadd{~\ref{fig:21a}. 
Each image shows bounding boxes for one or more detections. 
}\figurename\DIFadd{~\ref{fig:21b} presents the averaged false-positives per image (FPPI) vs. recall.
Our method is comparable to the-state-of-art method \mbox{%DIFAUXCMD
\cite{luo2010,jurie2004,ferrari2007,yarlagadda2010,lin2012}}%DIFAUXCMD
.
}\DIFaddend 

\begin{figure}[!t]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{images/fig21.jpg}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Representative detection results]{\includegraphics[width=0.95\linewidth]{images/fig21a.png}}\\
...
\DIFaddendFL \caption{Detection testing for the \DIFdelbeginFL \DIFdelFL{Weizzman horse }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{INRIA horses }\DIFaddendFL dataset.}
\label{fig:21}
\end{figure}
...
\DIFaddbegin \DIFadd{In }\tablename\DIFadd{~\ref{tab:2}, the comparison of time cost is shown with detailed statistics.
Both of the methods are implemented with MATLAB and run on the same Lenovo PC (with Intel i5 CPU, 4GB RAM).
}\DIFaddend We can see that our method is much less time consuming than the other method, 
which requires exhaustive searching.

\DIFaddbegin \begin{table}[!t]
\renewcommand{\arraystretch}{1.3}
\caption{\DIFaddFL{Comparison of time cost in minutes per image}}
\label{tab:2}
\centering
\scriptsize
\begin{tabular}{l|lllll}
\hline
\DIFaddFL{Category }& \DIFaddFL{Apple }& \DIFaddFL{Bottle }& \DIFaddFL{Giraffe }& \DIFaddFL{Mug }& \DIFaddFL{Swan }\\
\hline
\DIFaddFL{Our method (mean)}& \DIFaddFL{0.76 }& \DIFaddFL{5.34 }& \DIFaddFL{5.37 }& \DIFaddFL{6.27 }& \DIFaddFL{4.50 }\\
\DIFaddFL{Our method (min)}& \DIFaddFL{0.01 }& \DIFaddFL{0.01 }& \DIFaddFL{0.02 }& \DIFaddFL{1.63 }& \DIFaddFL{0.84 }\\
\DIFaddFL{Our method (max)}& \DIFaddFL{2.4 }& \DIFaddFL{13.0 }& \DIFaddFL{17.7 }& \DIFaddFL{15.6 }& \DIFaddFL{10.8 }\\
\hline
\DIFaddFL{Ma et al (mean)}& \DIFaddFL{67.1 }& \DIFaddFL{52.5 }& \DIFaddFL{59.2 }& \DIFaddFL{36.5 }& \DIFaddFL{38.6 }\\
\DIFaddFL{Ma et al (min)}& \DIFaddFL{27.3 }& \DIFaddFL{23.1 }& \DIFaddFL{15.9 }& \DIFaddFL{24.4 }& \DIFaddFL{22.3 }\\
\DIFaddFL{Ma et al (max)}& \DIFaddFL{151.8 }& \DIFaddFL{165.5 }& \DIFaddFL{251.4 }& \DIFaddFL{42.0 }& \DIFaddFL{60.4 }\\
\hline
\end{tabular}
\end{table}
...
\centering
\DIFdelbeginFL %DIFDELCMD < \subfloat[Giraffes in ETHZ]{\includegraphics[width=0.33\linewidth]{images/fig22a.jpg}}%%%
\DIFdelendFL \DIFaddbeginFL \subfloat[]{\includegraphics[width=0.33\linewidth]{images/fig22a.eps}}\DIFaddendFL \hfil%
\DIFdelbeginFL %DIFDELCMD < \subfloat[Apple logos in ETHZ]{\includegraphics[width=0.33\linewidth]{images/fig22b.jpg}}%%%
\DIFdelendFL \DIFaddbeginFL \subfloat[]{\includegraphics[width=0.33\linewidth]{images/fig22b.eps}}\DIFaddendFL \hfil%
\DIFdelbeginFL %DIFDELCMD < \subfloat[Swans in ETHZ]{\includegraphics[width=0.33\linewidth]{images/fig22c.jpg}}%%%
\DIFdelendFL \DIFaddbeginFL \subfloat[]{\includegraphics[width=0.33\linewidth]{images/fig22c.eps}}\DIFaddendFL \\%
\DIFdelbeginFL %DIFDELCMD < \subfloat[Bottles in ETHZ]{\includegraphics[width=0.33\linewidth]{images/fig22d.jpg}}%%%
\DIFdelendFL \DIFaddbeginFL \subfloat[]{\includegraphics[width=0.33\linewidth]{images/fig22d.eps}}\DIFaddendFL \hfil%
\DIFdelbeginFL %DIFDELCMD < \subfloat[Mugs in ETHZ]{\includegraphics[width=0.33\linewidth]{images/fig22e.jpg}}%%%
\DIFdelendFL \DIFaddbeginFL \subfloat[]{\includegraphics[width=0.33\linewidth]{images/fig22e.eps}}\DIFaddendFL %
...
\DIFaddbegin \DIFadd{From the engineering point of view,
...
\DIFaddbegin \DIFadd{The hypercolumn array is different from traditional representations such as edge map.
...
(d) The training set and appearance-based machine learning strategy for object detection under a natural environment can never exhaust all possible negative cases. Therefore, this strategy is really too expensive for its limited gain in generalization. The strategy of evidence accumulation and inference can avoid these costs\DIFaddbegin \DIFadd{.
}

\DIFadd{In this paper, we simulate one of the functions of hypercolumns in primary visual cortex,
...
It also helps us to find engineering implementation of image understanding algorithms based on multiple clues}\DIFaddend .
