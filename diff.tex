...
%DIF ADD bare_jrnl.tex       Fri Oct 30 16:56:22 2015
...
This is perhaps the most thorough methodbecause it matches curves based upon their overall geometrical tendencies rather than local numerical features. 
...
for dimensionality reduction, which is referred to as non-linear embedding ELM (NLE-ELM)}\DIFaddend .
...
\DIFaddbegin \DIFadd{The routes are represented with vectors which are provided as input to the NLE-ELM algorithm.
...
\DIFadd{We train the NLE-ELM model to recognize routes from a number $M$ of object categories.
...
The ELM is a single-hidden-layer feedforward neural network and the output function is defined in the following equation\mbox{%DIFAUXCMD
\cite{miche2010,huang2006}
}%DIFAUXCMD
.
}
\begin{equation}
...
\end{equation}
\DIFadd{where $\boldsymbol{h}(\boldsymbol{v})=\left[h_1(\boldsymbol{v}),h_2(\boldsymbol{v}),\cdots,h_L(\boldsymbol{v})\right]$
...
\end{split}
\end{equation}
\DIFadd{where $n$ is the number of training samples, 
...
\DIFadd{where $\boldsymbol{L_H}=\boldsymbol{I}_n-\boldsymbol{H'H'^T}(\gamma_g\boldsymbol{I}_n+\boldsymbol{H'H'^T})^{-1}$.
}

\DIFadd{Given the aforementioned route representation and NLE-ELM model,
...
\DIFaddend \begin{myalgorithm}{Algorithm 5.1 Route-Distance($r,t$)}
\begin{algorithmic}[1]
\REQUIRE \DIFdelbegin \DIFdel{$r$ is RouteClass curve and $t$ is TempClass curve.
}%DIFDELCMD < \STATE %%%
\DIFdel{rotate }\DIFdelend $r$ and $t$ \DIFdelbegin \DIFdel{so that their most extended direction is along the $x$-axis
}\DIFdelend \DIFaddbegin \DIFadd{are two routes.
}\DIFaddend \STATE \DIFdelbegin \DIFdel{sample $t$ uniformly to $N$ points $P_t=\{(x_i,y_i)|i=1,\cdots,N\}$
}\DIFdelend \DIFaddbegin \DIFadd{$\boldsymbol{v_r,v_t}\leftarrow$ vector representation of the routes
}\DIFaddend \STATE \DIFdelbegin \DIFdel{randomly initialize $M$ subintervals of $[0,length(r)]$
}%DIFDELCMD < \REPEAT
%DIFDELCMD <   \FOR{each subinterval $[p,q]$}
%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \DIFadd{Let $f$ denote the output function of the NLE-ELM model
}\DIFaddend \STATE \DIFdelbegin \DIFdel{sample $r$ in $[p,q]$ to $N$ points $P_r=\{(x'_i,y'_i)\}$
    }\DIFdelend \DIFaddbegin \DIFadd{$f(\boldsymbol{v_r})=\boldsymbol{h}(\boldsymbol{v_r})\boldsymbol\beta^{*}$
}\DIFaddend \STATE \DIFdelbegin \DIFdel{calculate dissimilarity $d(P_t,P_r)$ by solving least-squares
  }%DIFDELCMD < \ENDFOR
%DIFDELCMD <   \STATE %%%
\DIFdel{remove 2 subintervals with maximal dissimilarity
  }%DIFDELCMD < \STATE %%%
\DIFdel{add 2 subintervals by shrinking and extending the one with minimal dissimilarity
}%DIFDELCMD < \UNTIL{the minimal dissimilarity is steady}
%DIFDELCMD < \RETURN %%%
\DIFdel{the minimal dissimilarity
}\DIFdelend \DIFaddbegin \DIFadd{$f(\boldsymbol{v_t})=\boldsymbol{h}(\boldsymbol{v_t})\boldsymbol\beta^{*}$
}\RETURN \DIFadd{$||f(\boldsymbol{v_r})-f(\boldsymbol{v_t})||$
...
\DIFdel{~\ref{tab:1} lists a comparison of DR values obtained by our method and several }\DIFdelend \DIFaddbegin \DIFadd{We test the proposed method on the ETHZ dataset and the Weizzman horse dataset for validation.
...
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.963 }\DIFaddendFL & \DIFdelbeginFL \textbf{\DIFdelFL{0.77/0.80}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.859 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.75/}\textbf{\DIFdelFL{0.88}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.906 }\DIFaddendFL & \DIFdelbeginFL \textbf{\DIFdelFL{0.75/0.81}}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.972 }& \DIFaddFL{0.915 }\DIFaddendFL \\
...
\DIFaddend The Weizzman dataset of horse images is another popular dataset for object detection testing. 
...
Physiological inspirations are indispensable for designing a practical vision system\DIFaddbegin \DIFadd{.
...
This optimized design is imitating the result of natural evolution}\DIFaddend .
